...
public class NewsContent extends JFrame implements ActionListener{
	...
	public NewsContent(List<News> newsList,int position,String filePath) {
		...
		this.currentNews = this.newsList.get(position);
		tags = new Tags();
		tags.setType("");
		tags.setTheme("");
		tags.setSource("");
...
//		type4.setForeground(Color.white);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(newsList.size()>0 && position >= 0 && position < newsList.size()){
					...
					if(position >= 0 && newsList.size()>0){
						logger.info("点击上一篇打开未分类新闻--"+newsList.get(position).getTitle());
		...
		button_1.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(newsList.size()>0 && position >= 0 && position < newsList.size() && newsList.get(position).getTagIts().equals("true")){
					modifyList();
				}	
				logger.info("返回首页");
			...
			public void actionPerformed(ActionEvent e) {
				if(newsList.size()>0 && position >= 0 && position < newsList.size()){
					...
					if(position <= newsList.size()-1){						
						logger.info("点击下一篇打开未分类新闻--"+newsList.get(position).getTitle());
						typeGroup.clearSelection();
						themeGroup.clearSelection();
						sourceGroup.clearSelection();
						mediaImageGroup.clearSelection();
		...
		finish.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(newsList.size() > 0 && position >= 0 && position < newsList.size()){
					...
					ClassifiedNewsContent classifiedNewsContent = 
							new ClassifiedNewsContent(listData.classifiedNews,
									listData.findPosition(listData.classifiedNews, currentNews.getID()),filePath);
					classifiedNewsContent.setVisible(true);
				}
			}			
		});
	...
	public void modifyList(){
		logger.info("新闻---"+newsList.get(position).getTitle()+"被分类");
		newsList.get(position).setTags(tags);
